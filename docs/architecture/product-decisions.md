...
|---|---|---|---|
| After a corresponding event is successfully created, Google owns actual start, actual end, calendar placement, and event existence. | User edits in the calendar must remain authoritative. | Local planning state can become inconsistent and requires reconciliation. | Apply implemented; reconciliation remains planned. |
